...
def ean_lookup(query: str) -> str:
    ...
    def _format_summary(pairs, query=""):
        """Formata resumo de forma inteligente baseado na complexidade da query."""
        if not pairs:
            return None
        ...
        if len(query_words) <= 1:
            # Query genérica (ex: "frango") -> 3 resultados (economia de tokens)
            max_results = 3
        else:
    ...
    try:
        ...
        try:
            ...
            # Mantém até 10 resultados relevantes (score >= 1.0) para o LLM escolher
            top_relevant = [pn for pn, sc in sorted(scored, key=lambda x: x[1], reverse=True) if sc >= 1.0][:10]
            # Fallback: se não houver relevantes, use os primeiros 10 pares da busca vetorial
            used_pairs = top_relevant if top_relevant else ordered[:10]
            # Formato inteligente (3 ou 5 resultados dependendo da query)
            summary = _format_summary(used_pairs, query)
            ...
            summary = _format_summary(used_pairs, query)  # Passa query para formato inteligente
            if summary:
